...
def run_analysis(
    ...
    *,
    ...
    with TemporaryDirectory() as tmp:
        ...
        # Pre-analysis runs first to guide tool selection
        analyzers: List[tuple] = [
            (analyze_pre_scan, (image_path, output_dir)),
            ...
            # New robust analyzers
            (analyze_entropy_anomalies, (image_path, output_dir)),
            (analyze_jpeg_qtables, (image_path, output_dir)),
            (analyze_statistical_steg, (image_path, output_dir, deep_analysis)),
        ]
        ...
        if deep_analysis:
            ...
            try:
                ...
                if tools.get("outguess", {}).get("available"):
                    analyzers.append((analyze_outguess, (image_path, output_dir, password)))
                else:
                    update_data(
                        output_dir,
                        {
        ...
        analyzers.append((analyze_payload_unwrap, (image_path, output_dir), {"deep_analysis": deep_analysis}))
        ...
        def run_analyzer(analyzer):
            """Helper to run a single analyzer with error handling."""
            try:
                if len(analyzer) == 2:
                ...
                return analyzer[0].__name__, True
            except Exception as exc:  # pragma: no cover - defensive
                # Errors are already recorded inside analyzer, this keeps the pipeline alive.
                print(f"Analyzer {analyzer[0].__name__} failed: {exc}")
                return analyzer[0].__name__, False

        # Use ThreadPoolExecutor to run analyzers concurrently
        # Limit to 8 workers to avoid overwhelming the system
        max_workers = max(1, min(8, len(analyzers)))
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                analyzer_name, success = future.result()
                status = "completed" if success else "failed"
                print(f"Analyzer {analyzer_name}: {status}")
